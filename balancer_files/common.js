...
{$(this).select();});$("#embedisp").hide();conversionSelectClass();chf_utils();onkeyupValidationClass();closeModal();
$('.ec_calculator_gen').find("input[type = 'text']").focusout(function(){$("#dynErrDisp").html("");$("#dynErrDisp").hide();});
$('.ec_calculator_gen').find("input[type = 'text']").keyup(function(){
$("#dynErrDisp").html("");$("#dynErrDisp").hide();var buttonValue = $('.ec_calculator_gen').find("input[type = 'button']").val();
if (buttonValue == "Calculate" || buttonValue == "Convert" || buttonValue == "Result")
{$('.ec_calculator_gen').find("input[type = 'reset']").after( '<span class="errClass" id="dynErrDisp"></span>');
...
if(funcStrConv === false){setTimeout(function(){eval(butnOnclikFunName)}, 500);}}});});
...
{var easycurrency = new Array("&#2352;", "&#36;", "&#162;", "&#163;", "&#165;", "&#8355;", "&#8356;", "&#8359;", "&#128;");var easycurcountry=new Array("Rupee","Dollar","Cent","Pound","Yen","Franc","Lira","Peseta","Euro");var currencySel="<select onchange=changeCurrencySymbol(this)>";for(i=0;i<easycurrency.length;i++)
...
var easyValid=0;function easyCommonInputCalculation(textid,unitname,alertmsg)
{if(easyValid==0)
{var inputval=emptyValidation($("#"+textid+"hname").val(),alertmsg);if(inputval!=false||inputval==0)
{return getbasicval((textid+"unit"),inputval,unitname,'',0);}}
else{return false;}}
function easyCommonResultCalculation(textid,unitname)
{if(easyValid==1)
{return"";}
else
{if(isNaN(getbasicval((textid + "unit"),eval(textid + "result"),unitname,'result', 0)))
{return 0;}
else{return getbasicval((textid + "unit"),eval(textid + "result"),unitname,'result', 0);}}}
...
{easyValid=0;});});function onkeyupValidationClass()
{$(".easynumeric").numeric();$(".easyinteger").numeric(false,function(){alert("Integers only");this.value="";this.focus();});$(".easypositive").numeric({negative:false},function(){alert("No negative values");this.value="";this.focus();});$(".easypositive-integer").numeric({decimal:false,negative:false},function(){alert("Positive integers only");this.value="";this.focus();});}
...
function closeModal(){$("#dispCalcConts *").attr("disabled",false);$('#overlayLoader').css("display","none");$("#dispCalcConts").fadeTo("fast",1);}